...
2. **T15 起**：任務脈絡在 GitHub Issue/PR——用 `GH_TOKEN=$(gh auth token -u calvin99tw) gh issue view <N>` /
   `gh pr view <M>` 讀取（帳號機制見「開發流程」，勿用 `gh auth switch`）。
...
- **字級規範（Typography Scale）**：**禁止在 JS 樣板字串中使用 inline `style="font-size:...px"`**，所有字級一律透過 `<style>` 區塊中具名的 CSS class 定義，確保單一事實來源（前例：2026-07 Pool Card 字級調整時，因遺漏站內 inline font-size 覆蓋，導致部分文字未套用新字級——class 已改但 inline 優先權更高，悄悄蓋掉）。全站最小字級 **12px**（低於 12px 不符 Apple HIG Caption 1 / Material Design body-small 的建議下限），僅**主要數值**（金額/股數等核心數字）可用 16px 起（如 `.pool-stat-value`、`.s-value`）。全站已於 2026-07 完成一次稽核修復（見 #16），無遺留的 11px 或以下 inline 實例。
...
**帳號**：git 用 SSH 別名 `git@github.com-personal:calvin99tw/...` 推送；gh 用 token（HTTPS）開 PR / 讀 issue。這台機器 gh 同時存有公司帳號（`Calvin-Chang_tmemu`）。

**⚠️ 絕對不要用 `gh auth switch`**：那會改寫 `~/.config/gh/hosts.yml`，是**整台機器共用的全域設定**，會影響使用者其他終端機/session 正在跑的公司帳號操作，且切換是持久的，不會隨這次 session 結束還原。

**正確做法**：每個 `gh` 指令都用 `GH_TOKEN` 環境變數帶指定帳號的 token，只在**該次指令執行期間**生效，不觸碰全域 active 狀態、不影響其他任何 session：

```bash
GH_TOKEN=$(gh auth token -u calvin99tw) gh issue create ...
GH_TOKEN=$(gh auth token -u calvin99tw) gh pr create ...
GH_TOKEN=$(gh auth token -u calvin99tw) gh pr merge ...
```

`gh auth token -u <帳號>` 讀取該帳號已存在 keyring 中的 token（`gh auth login` 登入過即有），**不會**改變 active 帳號。已驗證：執行前後 `gh auth status` 的 active 帳號不變。
