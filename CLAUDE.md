...
- **臨時 bugfix／對帳排查改用 GitHub Issue 追蹤，不寫 `docs/bugfix-*.md`**：正式任務（T1~Tn）維持 `docs/Tx-spec.md` + `docs/Tx-summary.md` 慣例不變；但非任務型的臨時修復（例如資料對帳、重複記錄排查）改開 GitHub Issue 記錄排查過程，並在修復 commit 訊息或 PR 中引用該 issue（如 `Fixes #12`），讓問題脈絡直接與程式碼變更掛鉤，比另外開文件更容易追溯
